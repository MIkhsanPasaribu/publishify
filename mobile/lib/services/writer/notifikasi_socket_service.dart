...
class NotifikasiSocketService {
  ...
  Future<void> connect() async {
    if (_socket != null && _isConnected) {
      logger.i('[NotifikasiSocket] Already connected');
      return;
    }
    ...
    try {
      ...
      if (token == null || token.isEmpty) {
        logger.w('[NotifikasiSocket] ⚠️ No token found - cannot connect');
      ...
      logger.e('[NotifikasiSocket] Error connecting: $e');
      _isConnected = false;
      _connectionStatusController.add(false);
    }
  }
  ...
  void _setupEventListeners() {
    ...
    _socket!.onConnect((_) async {
      logger.i('[NotifikasiSocket] Connected successfully');
      _isConnected = true;
    ...
    _socket!.onConnectError((error) {
      logger.e('[NotifikasiSocket] Connection error: $error');
      _isConnected = false;
      _connectionStatusController.add(false);
    });
    ...
    _socket!.onDisconnect((_) {
      logger.i('[NotifikasiSocket] Disconnected');
      _isConnected = false;
      _connectionStatusController.add(false);
    });
    ...
    _socket!.on('reconnect_attempt', (attempt) {
      logger.i('[NotifikasiSocket] Reconnection attempt: $attempt');
    });
    ...
    _socket!.on('notifikasi_baru', (data) {
      ...
      try {
        ...
        logger.e('[NotifikasiSocket] Error parsing notifikasi_baru: $e');
      }
    });
    ...
    _socket!.on('notifikasi_count', (data) {
      ...
      try {
        ...
        logger.e('[NotifikasiSocket] Error parsing notifikasi_count: $e');
      }
    });
    ...
    _socket!.on('notifikasi_broadcast', (data) {
      ...
      try {
        ...
        logger.e('[NotifikasiSocket] Error parsing notifikasi_broadcast: $e');
      }
    });
  }
  ...
  Future<void> _joinUserRoom() async {
    ...
    try {
      ...
      _socket!.on('join_room_response', (data) {
        logger.i('[NotifikasiSocket] Join room response: $data');
      });
    } catch (e) {
      logger.e('[NotifikasiSocket] Error joining room: $e');
    }
  }
  ...
  void disconnect() {
    if (_socket != null) {
      logger.i('[NotifikasiSocket] Disconnecting...');
      _socket!.disconnect();
      _socket!.dispose();
      _socket = null;
      _isConnected = false;
