...
export const naskahApi = {
  ...
  /**
   * GET /naskah/kategori - Ambil daftar kategori
   */
  async ambilKategori(): Promise<ResponseSukses<Kategori[]>> {
    const { data } = await api.get<ResponseSukses<Kategori[]>>("/naskah/kategori");
    return data;
  },

  /**
   * GET /naskah/genre - Ambil daftar genre
   */
  async ambilGenre(): Promise<ResponseSukses<Genre[]>> {
    const { data } = await api.get<ResponseSukses<Genre[]>>("/naskah/genre");
    return data;
  },
};
