...
type TabKey = "editor" | "penulis" | "percetakan";
...
export default function KelolaPenggunaPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (e?.response?.status === 401) {
        ...
        const errorMessage = e?.response?.data?.pesan 
  ...
  const filteredPengguna = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      filtered = filtered.filter((p) => {
        const namaLengkap = `${p.profilPengguna?.namaDepan || ""} ${p.profilPengguna?.namaBelakang || ""}`.toLowerCase();
  ...
  const handleDelete = async (id: string) => {
    const confirm = window.confirm("Yakin ingin menghapus pengguna ini? Aksi ini tidak dapat dibatalkan.");
    if (!confirm) return;
      ...
      <div className="w-full max-w-7xl mx-auto px-6 sm:px-8 lg:px-12 py-6 sm:py-8 space-y-6 sm:space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 sm:gap-4">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-green-600">
                  {allPengguna.filter((p) => p.peranPengguna?.some((r) => r.jenisPeran === "penulis")).length}
                </p>
              </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-600 mb-1">Total Percetakan</p>
                <p className="text-3xl font-bold text-purple-600">
                  {allPengguna.filter((p) => p.peranPengguna?.some((r) => r.jenisPeran === "percetakan")).length}
                </p>
              </div>
              <div className="w-12 h-12 bg-purple-100 rounded-lg flex items-center justify-center">
                {getIconTab("percetakan")}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-4 mb-6">
          <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
            ...
            <div className="flex gap-2">
              {(["editor", "penulis", "percetakan"] as TabKey[]).map((tab) => (
                <button
                  key={tab}
              ...
              <tbody className="divide-y divide-gray-200">
                ...
                {!loading &&
                  filteredPengguna.map((pengguna) => (
                    <tr key={pengguna.id} className="hover:bg-gray-50 transition-colors">
                      {/* Pengguna */}
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 bg-gradient-to-br from-[#14b8a6] to-[#0d9488] rounded-full flex items-center justify-center text-white font-semibold">
                            {pengguna.profilPengguna?.namaDepan?.[0]?.toUpperCase() || pengguna.email[0].toUpperCase()}
                          </div>
                          <div>
                      ...
                      {/* Email & Telepon */}
                      <td className="px-6 py-4">
                        <p className="text-sm text-gray-900">{pengguna.email}</p>
                        <p className="text-sm text-gray-500">{pengguna.telepon || "-"}</p>
                      </td>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                          <button
                            onClick={() => handleToggleStatus(pengguna.id, pengguna.aktif)}
